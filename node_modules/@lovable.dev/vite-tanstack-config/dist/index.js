...
const DEFAULT_SERVER_OUTPUT_DIR = "dist/server";
...
function prerenderPreviewShim(state) {
	let root = process.cwd();
	let serverOutDir = DEFAULT_SERVER_OUTPUT_DIR;
	...
	return {
		...
		buildApp: {
			...
			handler: async () => {
				...
				if (!await exists(nodePath.join(outDir, NITRO_SERVER_ENTRY))) return;
				const basename = nodePath.basename(serverEntryName, nodePath.extname(serverEntryName));
				if (!basename || basename === "index") {
					logWarning(`Cannot prerender: the server entry "${serverEntryName}" would need a shim at ${DEFAULT_SERVER_OUTPUT_DIR}/index.js, which outranks nitro's own entry when publishing. Rename the entry to anything but \`index\`.`);
					return;
				}
